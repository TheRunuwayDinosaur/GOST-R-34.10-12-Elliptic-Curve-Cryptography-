...
void GOSTHashInit(TGOSTHashContext *CTX, uint16_t hash_size){
    ...
    if (hash_size == 256) memset(CTX->h, 0x01, BLOCK_SIZE);
    else memset(CTX->h, 0x00, BLOCK_SIZE);
...
static void GOSTHashLPS(uint8_t *state){
    uint64_t internal[8];
...
static void GOSTHashKeyShedule(uint8_t *K, int i){
    GOSTHashX(K, C[i], K);
    GOSTHashLPS(K);
}
